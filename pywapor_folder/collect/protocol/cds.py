...
import shutil
from pywapor_folder.general.processing_functions import save_ds
...
def download(folder, product_name, latlim, lonlim, timelim, variables, post_processors):
    ...
    if cdsapi_version < 70:
        ...
        url, key = pywapor_folder.collect.accounts.get("ECMWF")
    else:
        log.info(f"--> Using the new CDS beta.")
        url, key = pywapor_folder.collect.accounts.get("CDS")
