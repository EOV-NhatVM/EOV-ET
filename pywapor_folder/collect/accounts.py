...
from pywapor_folder.collect.product.LANDSAT import espa_api
...
def setup(account):
    ...
    folder = os.path.dirname(os.path.realpath(pywapor_folder.__path__[0]))
    filename = "secret.txt"
...
def cds_account(user_pw):
    ...
    try:
        ...
        fp = os.path.join(pywapor_folder.__path__[0], "test.zip")
        _ = c.retrieve(
