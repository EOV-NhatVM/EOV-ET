...
import os
import numpy as np
import pywapor_folder.et_look_v2_v3 as ETLook_v2_v3
import pywapor_folder.general as g
import datetime
...
def main(input_data, et_look_version = "v2", export_vars = "default", chunks = {"time_bins": -1, "x": 500, "y": 500}):
    ...
    ds["lai"] = ETLook.leaf.leaf_area_index(ds["vc"])
    ...
    #    ds["diffusion_index"] = ETLook.solar_radiation.diffusion_index(ds["trans_24"], diffusion_slope = ds["diffusion_slope"], diffusion_intercept = ds["diffusion_intercept"])
       ds["diffusion_index"] = ETLook.solar_radiation.diffusion_index(ds["trans_24"])
    ...
    ds["stress_vpd"] = ETLook.stress.stress_vpd(ds["vpd_24"]) # vpd_slope default
    ...
    if ds["u_24"].dtype == object:
        ...
        print(type(ds['u_24']))
    ds["ra_canopy_init"] = ETLook.neutral.initial_canopy_aerodynamic_resistance(ds["u_24"], ds["z0m"])

    # **windspeed blending height daily***********************************************************
    ds["u_b_24"] = ETLook.meteo.wind_speed_blending_height_daily(ds["u_24"])
    ...
    ds["r_soil"] = ETLook.resistance.soil_resistance(ds["se_root"])
    # **ETLook.resistance.soil_resistance***********************************************************
    ds["ra_soil_init"] = ETLook.neutral.initial_soil_aerodynamic_resistance(ds["u_24"])
    ...
    if len(ds.data_vars) == 0:
        ...
        # Trước khi transpose, kiểm tra xem các chiều có tồn tại không
        dims = list(ds.dims)
        # desired_order = [dim for dim in ["time", "y", "x"] if dim in dims]

        # ds = ds.transpose(*desired_order)

        ds = ds.transpose("time_bins", "y", "x") # set dimension order thee same for all vars.
