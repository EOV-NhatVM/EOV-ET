...
from pywapor_folder.collect.downloader import collect_sources
from pywapor_folder.general.compositer import time_bins
from pywapor_folder.general.processing_functions import adjust_timelim_dtype, func_from_string, open_ds, remove_ds, is_corrupt_or_empty, has_wrong_bb_or_period
...
--> Sharpening these variables:
    ...
    def from_name(cls, name):
        ...
        folder = os.path.realpath(pywapor_folder.__path__[0])
...
class Project():
    ...
    def set_passwords(self):
        ...
        for account in x:
            _ = pywapor_folder.collect.accounts.get(account)
    ...
    def run_pre_se_root(self, forced = False):
        if isinstance(self.se_root_in, type(None)) or forced:
            self.se_root_in = pywapor_folder.pre_se_root.main(self.folder, self.latlim, self.lonlim, self.period, sources = self.configuration.se_root)
        else:
    ...
    def run_se_root(self, se_root_version = "v3", export_vars = "default", chunks = {"time": -1, "x": 500, "y": 500}):
        self.se_root_out = pywapor_folder.se_root.main(
                                                self.se_root_in, 
    ...
    def run_pre_et_look(self, forced = False):
        if isinstance(self.et_look_in, type(None)) or forced:
            self.et_look_in = pywapor_folder.pre_et_look.main(self.folder, self.latlim, self.lonlim, self.period, sources = self.configuration.et_look)
        else:
    ...
    def run_et_look(self, et_look_version = "v3", export_vars = "default", chunks = {"time_bins": -1, "x": 500, "y": 500}):
        self.et_look_out = pywapor_folder.et_look.main(
                                                self.et_look_in, 
...
if __name__ == "__main__":
    ...
    project = pywapor_folder.Project(project_folder, bb, timelim)
