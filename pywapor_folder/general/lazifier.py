...
def etlook_decorator(func):
    ...
    def wrapper_func(*args, **kwargs):
        # Chỉ kiểm tra dtype nếu arg có thuộc tính đó
        check1 = np.all([
            not hasattr(arg, "dtype") or arg.dtype != object 
            for arg in args
        ])
        check2 = np.all([
            not hasattr(arg, "dtype") or arg.dtype != object 
        ...
        if check1 and check2:
            ...
            # Gán metadata nếu output là xarray DataArray
            if hasattr(x, "attrs"):
                x.attrs["calculated_with"] = [
                    getattr(arg, "name", str(type(arg))) for arg in args
                ]
                x.attrs["et_look_module"] = group
            ...
            missing_kwargs = [
                name for name, arg in kwargs.items() 
                if hasattr(arg, "dtype") and arg.dtype == object
            ]

            missing = missing_args + missing_kwargs
            log.warning(
                f"--> Insufficient data found for `{func.__name__}`, missing [`{'`, `'.join(missing)}`]"
            )
